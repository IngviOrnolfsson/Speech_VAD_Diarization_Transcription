...
def filter_low_energy_segments(
    ...
    Args:
        ...
        interactive_threshold: If True, interactively adjust threshold with audio examples.
    ...
    if segment_stats:
        ...
        kept_count = sum(1 for stat in segment_stats if stat['energy_db'] >= threshold_db)
        cut_count = len(segment_stats) - kept_count
        percentage_cut = (cut_count / len(segment_stats)) * 100
        print(f"Energy filtering: Max {reference_db:.1f} dB, threshold {threshold_db:.1f} dB (margin {energy_margin_db:.1f} dB)")
        print(f"Segments: {len(segment_stats)} total, {kept_count} kept, {cut_count} cut ({percentage_cut:.1f}%)")
...
def _apply_energy_filtering(segment_stats: list, energy_margin_db: float) -> pd.DataFrame:
...
def _interactive_energy_filtering(
    ...
    while True:
        ...
        # Count segments
        kept_count = sum(1 for stat in segment_stats if stat['energy_db'] >= current_threshold)
        cut_count = len(segment_stats) - kept_count

        # Print current status
        print(f"\nMax energy: {max_energy_db:.1f} dB")
        print(f"Current threshold: {current_threshold:.1f} dB (margin: {current_margin:.1f} dB)")
        print(f"Segments kept: {kept_count}, cut: {cut_count}")

        # Find examples for boundary segments
        kept_stats = [stat for stat in segment_stats if stat['energy_db'] >= current_threshold]
        cut_stats = [stat for stat in segment_stats if stat['energy_db'] < current_threshold]
        ...
        if kept_stats and cut_stats:
            ...
            print(f"- Loudest cut segment: {loudest_cut_path} ({loudest_cut['energy_db']:.1f} dB)")
            print(f"- Quietest kept segment: {quietest_kept_path} ({quietest_kept['energy_db']:.1f} dB)")
            print(f"Energy difference: {quietest_kept['energy_db'] - loudest_cut['energy_db']:.1f} dB")

        # Ask user for new threshold
        while True:
            response = input("\nEnter new threshold in dB (or 'k' to keep current): ").strip()
            if response.lower() == 'k':
